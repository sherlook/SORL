...
class Generator(nn.Module):
    ...
    Parameters:
        ...
        obs_dim: The dimension of observation
    Output：
        action:
    """
    def __init__(self, act_dim, obs_dim, latent_dim):
        super(Generator, self).__init__()
        self.latent_dim = latent_dim
        self.act_dim = act_dim
        self.obs_dim = obs_dim
        self.model = nn.Sequential(
            nn.Linear(self.latent_dim+self.obs_dim, 128),
            nn.BatchNorm1d(128),
            nn.GELU(),
            nn.Linear(128, 128),
            nn.BatchNorm1d(128),
            nn.GELU(),
            nn.Linear(128, np.prod(self.act_dim, dtype=np.int32)),
            nn.Sigmoid()
        )
    ...
    def forward(self, z, obs):
        ...
        z = torch.cat([z, obs], axis=-1)    #拼接在一起，不一定最好，可以尝试不同的特征构造方法
        # print("z.shape:", z.shape)
        # print("obs.shape", obs.shape)

        output = self.model(z)
        action = output.reshape(z.shape[0], self.act_dim)
        return action
...
class Discriminator(nn.Module):
    ...
    def __init__(self, act_dim, obs_dim):
        ...
        self.model = nn.Sequential(
            nn.Linear(np.prod(self.act_dim, dtype=np.int32)+self.obs_dim, 128),
            nn.GELU(),
    ...
    def forward(self, action, state):
        prob = self.model(torch.cat([action, state], axis=-1))
        return prob
