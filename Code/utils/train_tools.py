...
def evaluate(agent, episode_num, seed_offset=100, show=False):
    ...
    normalized_score = agent.env.get_normalized_score(avg_reward)

    print("=====> evaluate {} episode <===> average step length: {:.2f} <===> average reward: {:.2f}<===> normalized score: {:.2f} <=====".format(episode_num, avg_length, avg_reward, normalized_score))
    print("---------------------------------------------------------------------------------------------------")
    return avg_reward, avg_length, normalized_score


def evaluate_unity(agent, episode_num, show=False):
    if show:
